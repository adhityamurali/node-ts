...
import { CartRepository, WarehouseRepository } from '@source/repositories';
import { ProductRepository } from '@source/repositories';
import { Cart } from '@source/models/cart';
import { CartService } from '@source/services/Cart';
...
export class CartController extends BaseController {
  ...
  public async getCheckoutValue(req: Request, res: Response): Promise<void> {
    try {
      ...
      const  {distance_in_kilometers}  = await this.warehouseRepository.getDistance(postal_code)
      const cart = await this.cartRepository.getAllCarts();
      const result = await cartService.getCheckOutValue(cart as Cart[],distance_in_kilometers,postal_code);
      res.status(201).send(result);
    } catch (error) {
      this.sendCreateUpdateErrorResponse(res, error);
    }
  ...
  public async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      const cart = await this.cartRepository.getAllCarts();
      const productDetails = await this.productRepository.getProductById(productId)
      if (!productDetails) {
         res.status(500).json({
            type: "Not Found",
            msg: "Invalid request"
      ...
      if(cart?.length !== 0){
        const cartData = await cartService.appendToCart(productDetails!,productId,quantity,cart as Cart[])
        const result = await this.cartRepository.updateItemToCart(cartData.id,cartData);
        res.status(201).send(result);
      }else{
        const cartData: Cart = {
          items: [{
